...
const Dashboard = () => {
  ...
  const currentScore =
    habits.length > 0 ? Math.round((perfectedToday / habits.length) * 100) : 0;
  ...
  if (!isLoggedIn || !storedUser)
    return (
      <div className="h-screen bg-[#1a164d] flex items-center justify-center text-white font-bold">
        Please Login.
      </div>
    );
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gradient-to-br from-[#1a164d] via-[#2e1065] to-black text-white overflow-hidden">
        ...
        {hasMilestone && (
          <div className="relative overflow-hidden mb-8">
            ...
            <div className="relative bg-black/40 border border-white/10 p-6 rounded-3xl flex items-center gap-6">
              <div className="bg-orange-500 w-12 h-12 rounded-xl flex items-center justify-center text-xl shadow-lg">
                🔥
              </div>
              <div>
                <h3 className="text-lg font-bold">7-Day Streak Club!</h3>
                <p className="text-gray-400 text-xs">
                  You're maintaining incredible consistency. Keep the fire
                  burning!
                </p>
              </div>
            </div>
      ...
      <AddHabitModal
        ...
        onAdd={addHabit} // Receives (title, desc, target, cat, isEveryday, unit)
        onUpdate={updateHabit} // Receives (id, title, desc, target, cat, isEveryday, unit)
