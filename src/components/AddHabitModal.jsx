...
const AddHabitModal = ({ isOpen, onClose, onAdd, onUpdate, editingHabit }) => {
  ...
  const [unit, setUnit] = useState("mins"); // New state
  ...
  // Auto-update unit when category changes (for new habits)
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-4">
            <div className="flex-[1]">
              ...
              <input type="number" min="1" className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 mt-2 outline-none focus:border-indigo-500" value={target} onChange={(e) => setTarget(e.target.value)} required />
            </div>
            <div className="flex-[1]">
          ...
          <div className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl cursor-pointer" onClick={() => setIsEveryday(!isEveryday)}>
            <input type="checkbox" checked={isEveryday} onChange={(e) => setIsEveryday(e.target.checked)} className="w-5 h-5 accent-indigo-500" />
            <label className="text-sm text-gray-300">Repeat this goal every day</label>
          </div>

          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 py-4 rounded-2xl bg-white/5 font-black text-xs uppercase hover:bg-white/10">Cancel</button>
            <button type="submit" disabled={status !== "idle"} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase ${status === "success" ? "bg-green-500" : "bg-indigo-600 shadow-xl shadow-indigo-600/30"}`}>
