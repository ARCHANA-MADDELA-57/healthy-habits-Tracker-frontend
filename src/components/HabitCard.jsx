...
const HabitCard = ({ habit, onIncrement, onDecrement, onEdit, onDelete }) => {
  ...
  const handleDelete = () => {
    onDelete(habit.id);
    toast.error(`${habit.title} removed!`, {
      icon: "🗑️",
  ...
  return (
    <motion.div
      ...
      className={`relative group bg-white/5 backdrop-blur-md border ${
        isCompleted ? "border-green-500/50 shadow-[0_0_20px_rgba(34,197,94,0.1)]" : "border-white/10"
    ...
    >
      ...
      <div className="w-full h-2 bg-black/20 rounded-full overflow-hidden mb-6">
        <motion.div
          ...
          className={`h-full ${isCompleted ? "bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]" : "bg-indigo-500"} transition-all`}
        />
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`flex-[2] py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${
            isCompleted ? "bg-green-500/20 text-green-500 cursor-not-allowed border border-green-500/30" : "bg-indigo-600 hover:bg-indigo-500 shadow-lg active:scale-95"
          }`}
        >
        ...
        <div className="flex gap-1 ml-auto">
          ...
          <button onClick={handleDelete} className="p-3 bg-red-500/10 hover:bg-red-500/20 rounded-2xl transition-all text-red-500">🗑️</button>
        </div>
      </div>
    </motion.div>
  );
